...
export const getFacebookPosts = async (req: Request, res: Response) => {
  try {
    ...
    if (user.facebookTokenExpiry && user.facebookTokenExpiry < new Date()) {
      return res.status(401).json({ message: 'Facebook token expired, please reconnect your account' });
    }
    ...
    const postsResponse = await axios.get(
      `https://graph.facebook.com/v18.0/${pageId}/posts`,
      {
        params: {
          ...
          fields: 'id,message,created_time,permalink_url,attachments{type,url},insights.metric(post_impressions,post_reactions_by_type_total)',
          limit: 10,
        },
      }
    );
    ...
    const formattedPosts = posts.map((post: any) => {
      ...
      if (post.insights?.data?.[1]?.values?.[0]?.value) {
        ...
        engagement.reactions = Object.values(reactions).reduce((a: number, b: number) => a + b, 0);
      }
...
export const getLinkedInPosts = async (req: Request, res: Response) => {
  try {
    ...
    const formattedPosts = posts.map((post: any) => {
      ...
      if (post.specificContent?.['com.linkedin.ugc.ShareContent']?.media?.[0]?.originalUrl) {
        mediaUrl = post.specificContent['com.linkedin.ugc.ShareContent'].media[0].originalUrl;
      }
...
export const createPost = async (req: Request, res: Response) => {
  try {
    ...
    const invalidPlatforms = platforms.filter((platform: string) => !validPlatforms.includes(platform));
        ...
        if (platform === 'FACEBOOK' && user.facebookToken) {
          ...
          if (mediaUrl) {
            if (mediaUrl.startsWith('http')) {
              ...
              errors.push({ platform, message: 'Direct image upload not implemented yet' });
              continue;
            }
          }
    ...
    if (postResults.length === 0 && errors.length > 0) {
      return res.status(400).json({ message: 'Failed to create posts', errors });
    }
