...
export const initiateOAuthFacebook = async (req: Request, res: Response) => {
  ...
  const authUrl = `https://www.facebook.com/v18.0/dialog/oauth?client_id=${env.FACEBOOK_APP_ID}&redirect_uri=${redirectUri}&scope=${scopes.join(',')}&response_type=code&state=facebook`;
...
export const facebookCallback = async (req: Request, res: Response) => {
  try {
    ...
    return res.redirect(`${env.FRONTEND_URL}/dashboard/settings?platform=facebook&status=success`);
    
  } catch (error) {
    const err = error as Error & { response?: { data?: any, status?: number } };
    console.error('Facebook OAuth error:', err.response?.data || err.message);
    return res.redirect(`${env.FRONTEND_URL}/dashboard/settings?platform=facebook&status=error&message=${encodeURIComponent(err.message || 'An error occurred')}`);
  }
};
...
export const linkedInCallback = async (req: Request, res: Response) => {
  try {
    ...
    // Redirect to frontend
    return res.redirect(`${env.FRONTEND_URL}/dashboard/settings?platform=linkedin&status=success`);
    
  } catch (error) {
    const err = error as Error & { response?: { data?: any, status?: number } };
    console.error('LinkedIn OAuth error:', err.response?.data || err.message);
    return res.redirect(`${env.FRONTEND_URL}/dashboard/settings?platform=linkedin&status=error&message=${encodeURIComponent(err.message || 'An error occurred')}`);
  }
};
...
export const disconnectSocialAccount = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ message: 'Server error disconnecting account' });
  }
};
